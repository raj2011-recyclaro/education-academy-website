...
import { deleteMasterclass, getAdminMasterclasses, updateMasterclassStatus } from "../../lib/api";
...
export default function AdminMasterclassesPage() {
  ...
  const load = () => {
    getAdminMasterclasses({ status })
      .then((data) => setCourses(data))
      .catch((requestError) => setError(requestError.message || "Could not load masterclasses."));
  };
  ...
  const togglePublish = async (course) => {
    try {
      await updateMasterclassStatus(course.id, course.visibilityStatus === "published" ? "draft" : "published");
      load();
    } catch (requestError) {
  ...
  return (
    <>
      <div className="admin-header">
        ...
        <Link className="button" to="/admin/masterclasses/new">+ New masterclass</Link>
      </div>
      ...
      <div className="admin-table-wrap">
        <table className="admin-table">
          ...
          <tbody>
            {courses.map((course) => (
              <tr key={course.id}>
                ...
                <td className="actions">
                  <Link className="link-button" to={`/admin/masterclasses/${course.id}`}>Edit</Link>
            ...
            {courses.length === 0 && <tr><td colSpan={5}>No masterclasses yet.</td></tr>}
          </tbody>
        </table>
      </div>
    </>
